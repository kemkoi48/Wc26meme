...
| S7 | Options (long calls/puts) | DRAFT, option_level_2 confirmed live | Passarelli + own catalyst-mismatch research | `option_math.py` + `option_scanner.py` built, unit-tested (`test_option_math.py`). Four contracts graded across two live runs (ONDS, LUNR, STNE, NKTR), all four rejections, zero trades placed. |
| S8 | Verified Catalyst Momentum | DRAFT, n=1, not yet proven | Not a book — reverse-engineered from this week's own trades and rejections | Written 2026-08-15 after noticing S1–S7 had contributed zero trades while the ad hoc screen carried the account. Formalizes the catalyst check + float-turnover disqualifier that separated HHS from every rejected pump name this week. Float-turnover threshold not yet backtested against a real sample — see S8's own "what is not yet true" section before trusting it. |
