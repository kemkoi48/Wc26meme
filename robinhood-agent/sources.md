...
## 2026-09-14 ~6:35am ET — Premarket watchlist rebuilt to "September 14"; risk-off tape

Market-holiday guard: SPY fresh premarket prints (last_non_reg_trade_time 10:32:21Z, bid/ask updating 10:34:53Z) -- real session, market open.

**REPO RECOVERY NOTE, logged because it nearly cost this cycle's work:** the container was re-provisioned over the weekend and the local clone came up with the working tree checked out at `ccac151` ("Delete netlify/functions directory") -- an old commit from `origin/main`'s line of history, with the entire `robinhood-agent/` directory absent. Nothing was lost: verified `git status` clean, confirmed via `git merge-base --is-ancestor ccac151 origin/main` that the three local-only commits were all contained in origin/main (so nothing unique to lose), fetched (which showed `+ ccac151...1de71e1 (forced update)` -- a stale local remote-tracking ref, not a remote rewrite), and confirmed all five of Friday's commits (fb9798f, ac4f15a, af23304, 86c040d, 1de71e1) still reachable on the remote with 1de71e1 as origin HEAD. Reset the local branch to origin; sources.md restored intact at 5060 lines. **Lesson: on a fresh container, verify HEAD matches the remote working branch BEFORE assuming the repo state is current -- and check whether anything is unique to local before resetting, rather than resetting reflexively.**

**Tape context (this drove every decision below):** broad risk-off, concentrated in AI/semis. SPY $758.82 (-0.72% vs Friday's $764.29 close), QQQ -1.62%, and the semi/AI-infra complex hit much harder -- SOXL -13.2%, AMD -5.1% ($489.63 vs $516.13), CRWV -6.6%, VRT -5.8%, CRDO -5.8%, NOK -10.2%, ORCL -3.3%. TLT +0.17% and defensives green (UNH +1.1%, MCD +0.74%) confirm the rotation is real rather than a single-name story.

**DROPPED — TNON** ($5.25, -11.5%, Warrior scan). Fourth straight session of decay from the $9.35 spike high (Thu) -> $5.93 (Fri close) -> $5.25 now. Move fully played out, chatter gone. Clean curation drop.

**ADDED — RUM** ($8.33, +16.3%, ~2.3M premarket shares, 35K watchers). Real catalyst, but with a caveat that matters more than the headline: MT Newswires (05:34 ET) reports The Information's weekend story that **Anthropic** is the previously-unnamed customer in RUM's $13.7B six-year Maysville, GA cloud-compute deal. **The deal itself is NOT new** -- RUM disclosed it in an August SEC filing and already rallied on it then (Benzinga 08-24 +6.6% to $9.64; 08-25 +11.2% to $10.41, "approaching its 52-week high of $10.54"). What is new today is only the customer's *identity*, and it is **unconfirmed**: MT Newswires carries its own "may include rumor and speculation, accuracy not guaranteed" disclaimer, and both Anthropic and RUM declined to comment. Note also RUM at $8.33 is still *below* its August high of $10.54 made on this same contract. Already fading: premarket peak ~$9.09 per the Stocktwits spark series, now $8.33 (~-8% off the high), and a trader flagged at 10:32 "it's already dropping from 20%." Added as a WATCH (real volume, real chatter, real news event), explicitly **not** as a clean setup -- recycled catalyst + unconfirmed + already fading off the premarket high. **Disclosure carried to the user in the message: Anthropic is the company that makes this agent; I hold no non-public information about it and relied entirely on the same public reporting the user can see.**

**ADDED — SRRK** (Scholar Rock, +11.9% to $62.00 premarket, per Benzinga). **FDA approval of ISEMBYLD (apitegromab-mstn)**, described as the first and only muscle-targeted treatment for Spinal Muscular Atrophy. Hardest, most binary catalyst on the board today. Invisible to both saved scans because of the known $1-20 / $2-20 `Last` ceiling defect (same failure mode as the BIAF miss at $20.35 on 2026-09-04) -- caught only via the news sweep, which is exactly why the multi-angle step exists.

**ADDED — CRBP** (Corbus, +19.3% to $9.69 premarket). Company hosts a **conference call TODAY (Sept 14)** to discuss topline data for CANYON-1 -- a dated, scheduled, binary event landing during today's session. Appeared on Friday's Early Momentum Ignition scan at $8.17 (-0.8%), so this is a genuine overnight development, not a continuation.

**KEPT — ORCL** ($145.34, -3.3%). Now trading *below* where it was added on the 09-11 EPS beat: Friday's range was $166.00 high / $149.84 low, so the entire earnings pop has round-tripped and inverted. Kept deliberately rather than dropped -- it is the bellwether of the AI-capex unwind driving the whole tape, with 71K watchers and heavy real volume. Live, just with an inverted thesis.

**KEPT — nuclear/uranium core (LEU, SMR, OKLO, UUUU, CCJ)**, all modestly red with no idiosyncratic news, consistent with beta to the risk-off tape: LEU $152.00 (-0.2%), SMR $8.43 (-2.1%), OKLO $35.00 (-3.4%), UUUU $12.73 (-1.7%), CCJ $95.20 (-1.5%). Standing theme, not today's action.

**Considered and deliberately NOT added — VSME** ($1.22, +14.0%, float 2.6M, 10.6M shares traded), the only green name in the Warrior scan (6 matches total, everything else red). Second consecutive up day (Benzinga: +18.7% premarket "after gaining 16% on Friday") but **no stated catalyst in any news source checked**, sub-$2, and the scan's own Relative-volume field reads a flat "1" -- the known-broken premarket placeholder, so elevated volume could not be confirmed from it. Under the WETO dilution rule this profile (sub-$5 microcap, small float, multi-day run, no catalyst) needs a `get_sec_filing_index` check before it earns a place, which was not run this cycle. Left off with the reason stated rather than added on price action alone.

**BDRX vindication, logged for the record:** BDRX -12.3% at $1.14 premarket, and Benzinga supplies the reason -- the company "announced interim results for the six months ended June 30, 2026." After a full week of flagging BDRX as a real move with *no* catalyst and heavy dilution (13 prospectus filings since 07-01), plus relaying a peer trader's "rug pull week" warning, the name finally has a dated catalyst and it is negative. The caution was correct; no watchlist action.

**Earnings check (next 3 days, high-market-cap):** nothing material for today -- KMTS today pm, FPS/TCOM/PS on 09-15 (TCOM pm is the only real large-cap), LEN 09-16 pm. No earnings catalyst feeds today's list.

Final list (9): ORCL, LEU, SMR, OKLO, UUUU, CCJ, RUM, SRRK, CRBP. Renamed in place via update_watchlist (no second list created).

## 2026-09-14 ~7:10am ET — Momentum-scanner ALERT: PMI, real 45x volume, clean filings, no catalyst

Market-holiday guard: SPY premarket print 11:10:06Z ($759.59, still -0.6%), market open.

**The broken-RVOL substitution mattered a lot this cycle and is worth recording concretely.** Every row in Early Momentum Ignition (94 matches) returned `Relative volume: 1` — the known flat placeholder — while `Relative volume (1, 1H)` returned obvious garbage: KZIA 170,395x, ZUMZ 616,216x, CVV 422,843x, APUR 13,799x. Taking any of those at face value would have been the exact RULE ZERO failure this job guards against. Self-computed real RVOL from `get_equity_historicals` (bounds=extended, hourly) instead, comparing today's cumulative 08:00-11:00 UTC premarket volume against the same window averaged over the prior 5 sessions (09-04, 09-08, 09-09, 09-10, 09-11).

**PMI (Picard Medical) — ALERTED.** $5.96, **+11.8%** (prior close $5.33), float 1.55M.
- **Real premarket volume, self-computed: 44,281 shares today (13,075 + 5,290 + 25,916 across the 4am/5am/6am ET bars) vs. a 977-share average over the same window on the prior 5 sessions -> ~45x.** Genuinely elevated, verified from real bars, not the broken field. Prior-session comparables were 0 / 2,085 / 1,792 / 111 / 898 shares, so this is a step-change in participation, not noise.
- **Timing: still live, not already-printed.** Premarket high $6.18 was set in the 4-5am ET bar (~2-3h ago), but the most recent completed hour (6-7am ET) opened $5.62, ran to $5.9997 and closed $5.9988 — i.e. it closed at its own bar high, on the session's heaviest volume (25,916 of the 44,281 total). Rebuilding toward the high rather than decaying away from it. Currently ~3.6% off the premarket high.
- **Catalyst: NONE. Flagged explicitly as "no catalyst found — price action only."** Stocktwits message volume is EXTREMELY_LOW (score 24, 1,531 messages, only 1,298 watchers) and every post is bare ticker-spam or a price target with no reasoning ("$PMI $11 soon", "$PMI $12", repeated bare "$PMI" posts from one user, one screener-vendor promo). No news, no filing, no event named by anyone. Note also the Stocktwits sentiment block self-contradicts (bull_pct 100 / bear_pct 0 but label "BEARISH", score 43) on a tiny sample — disregarded rather than reported as signal.
- **Dilution check: CLEAN, and that is the unusual part.** `get_sec_filing_index` since 07-01 returns exactly two filings — an 8-K and a 10-Q, both 2026-08-19, both routine. **No prospectus, no 424B, no ATM, no shelf.** This is a genuine outlier for this cohort: the 2026-09-04 sweep found live dilution mechanics on FIVE of six low-float movers checked, BDRX carried 13 prospectus filings since 07-01, and WETO had a $75M ATM on file while being flagged as a bounce. PMI carrying none is the single most favorable fact about it, and is why it was alerted despite having no catalyst.

**AEHL — real volume, REJECTED on timing (already printed).** $8.45, +8.2%, float 1.15M. Self-computed premarket RVOL is also genuinely huge: 140,090 shares today (114,198 + 20,664 + 5,228) vs. a 2,281 average over the same window on the prior 5 sessions -> **~61x**, even higher than PMI's. But the price action disqualifies it: the 4-5am ET bar opened $9.81, spiked to **$10.39**, and closed $8.50 — a violent spike-and-fade inside a single hour. Now $8.45, i.e. **-18.7% off the premarket high**, with volume collapsing hour over hour (114,198 -> 20,664 -> 5,228). Textbook already-printed; high was hours ago and has not been retested while participation drains. No alert. Recorded because the volume number alone would have looked alert-worthy in isolation — timing is what rejected it, exactly as the TNON $9.35 spike-fade precedent established.

Nothing else cleared. Of the >5% movers: CRBP ($8.59, +5.8%) was already added to the watchlist at the 6:35am rebuild on its CANYON-1 topline call today, and has since faded from its $9.69 premarket print — no new alert. VSME ($1.2187, +13.9%) unchanged from the 6:35am assessment (no catalyst, dilution check still not run) — no material change, still not added. TPET (+11.2%, $2.09), DBGI (+6.5%, $6.39, sub-1M float), SKYQ (+5.8%, $3.29) noted but not worked up; none showed a catalyst and none was the cycle's largest mover. Warrior scan produced only one green name (VSME) out of six.

User message sent (PMI only), with the standing >5-minute-hold reminder attached per protocol.

## 2026-09-14 ~8:11am ET — Momentum-scanner ALERT: FTFT third run at $3.40, fresh high but THINNER volume

Market-holiday guard: SPY premarket print 12:10:35Z ($759.45, -0.6%), market open.

**PMI (alerted 7:10am) correctly NOT re-alerted.** Now $5.90 (bid $5.71 / ask $5.91) vs $5.96 an hour ago; premarket high still $6.18, no new high, no volume expansion. Per step 2's "skip anything already alerted unless materially changed," no second message. It has not broken down either — just flat.

**FTFT (Future FinTech) — ALERTED, with a real caveat.** $3.32-3.34, **+15.3 to +16.0%** (prior close $2.88), float 7.94M. One hour ago this was **-4.2% at $2.76** — it flipped from red to strongly green inside a single premarket hour.
- **Timing is the best of any name today: it is making fresh highs AS THIS RAN.** The 11:00 UTC (7-8am ET) bar opened $2.68, ran to $3.29, closed $3.112 on 764,524 shares; price is now $3.34, i.e. above that bar's high. Not a fade, not already-printed — an in-progress breakout, which is the rarest and most actionable state this job looks for.
- **BUT the volume story is the opposite of PMI's, and it is the reason this is flagged rather than recommended.** Self-computed premarket volume (08:00-12:00 UTC): **812,408 shares today** vs. the same window on prior sessions -- 09-04: 4,208 / 09-08: 3,775 / 09-09: 9,791 / **09-10: 3,761,189** / **09-11: 3,127,058**. Against the pre-run baseline (the three quiet days) that is ~137x. But against the 5-session mean (1,381,204) it is **0.59x — today's push is running on roughly a QUARTER of the premarket participation that drove each of the last two sessions.** Price is making a higher high on materially thinner volume. That is a bearish divergence, not a confirmation, and it was stated plainly to the user rather than buried.
- **This is attempt #3 at the same ceiling.** FTFT ran to $3.55 on 09-09, to $3.40 on 09-11 (logged that day as already-printed with no catalyst), faded both times, and is now back at $3.34 pressing the same $3.40 level.
- **Catalyst: NONE, again.** Stocktwits message volume EXTREMELY_HIGH (92) and EXTREMELY_BULLISH (88% bull), but every post is a bare price target or hype — "3.40 for the next leg up", ">3.31>3.49>3.81", "4 plus", "not selling less than 200 percent from here". No news, no filing, no event named by anyone, same as the 09-11 check.
- **Pattern worth recording: the same promotional account is rotating between the names this scanner surfaces.** The account "TempleofBoom"/TOB posted "$FTFT paying us again / Join us, link in bio" at 11:59 UTC today. That is the same account that posted the XHLD pump on Friday 09-11 ("13.47 is next res then we could see 20.00 / Join TOB") during the 3:09pm XHLD alert. Two different low-float names, three days apart, same promoter driving chatter. Useful as a standing tell, and relayed to the user as that account's behavior, not as this account's claim about causation.

**VSME — REJECTED on timing, despite an eye-watering volume ratio.** $1.24-1.27, +18.7%. Self-computed premarket volume 1,645,218 shares today vs. a 94-share average over the same window on the prior 5 sessions -- nominally **~17,400x**, but that figure is close to meaningless because the base is essentially zero (this name had literally no premarket participation before Friday; four of the five comparison days are 0). The honest statement is "from nothing to 1.6M shares," not a clean multiple. Rejected anyway on price: today's high $1.48 was set in the 4-5am ET bar (~3h ago), price is now $1.24-1.27 = **-14% to -16% off that high**, with volume draining hour over hour (1,037,867 -> 311,410 -> 126,451 -> 169,490). Same already-printed/spike-fade shape as AEHL at 7:10am. Also note the move originated in Friday's POST-market session (the 20:00 UTC bar traded 2.66M shares, $1.01 -> $1.30 high) — an after-hours pump now bleeding out.
...
### Momentum scan 2026-09-25 9:17 ET (13:17Z), premarket into the open
- ALERT APUS $7.00-7.03 (+42% vs 4.92 close): real volume returning — 8:50am ET bar 390K shares (6.32->6.87, high 7.20), 8:55am bar 582K shares (high 7.25), pulled back to 6.54, then another push to high 7.20 at 9:10-9:13am ET on 225K shares. This is APUS's first formal alert today (checked and passed at 7:17/8:17, both times faded/flat) — now retesting/matching its overnight highs on real fresh volume, still within the igniting window. Same catalyst: MindWave/AQUAEChain deal (Benzinga, 9/24).
- INLF $6.38: faded further from the 8:17 peak ($6.80+), no re-alert.
...
## 2026-09-25 12:17 ET — scanner cycle (quiet, no alert)
Ignition 27 / Warrior 15 / safety-net 13. SPY live (771.59 @ 16:16:46Z).
- MSGY: MATERIALLY CHANGED again. Reopened ~12:00pm ET at $9.52 after the halt, spiked to $11.42 (16:00-16:05 bars), now fading to $8.30-8.87 on continued heavy volume. Same single Benzinga mention from 11:03am ET ("shot up 110%", no reason given) is still the only press. Stocktwits is now openly calling it a pump/rug: "China algo here. Big rug pulls and fast rips," "fuck this scam took all my money." No catalyst found, confirmed pump-and-dump pattern playing out live. GATED, not alerted.
...
## 2026-09-25 15:17 ET — scanner cycle (quiet, no alert; partial data outage)
Ignition 60 / Warrior 17 / safety-net 17. SPY live (770.98 @ 19:16:38Z).
*** DATA OUTAGE THIS CYCLE: get_equity_news (Robinhood/Benzinga) errored "tool cannot be found" on every call, including after a ToolSearch reload. Stocklake get_stock_news is back on its 25-call/day guest limit (also mis-resolved AIB -> ATAI, a different company - do not trust that result). Catalyst checks below are Stocktwits-only, which per the hard gate can NEVER itself confirm a catalyst. Treating all three as unconfirmed/gated rather than guessing. ***
- AVX (Avax One Technology) $6.05 (+11%, float 6.6M): Stocktwits is pure NAV/DAT (digital-asset-treasury) speculation ("NAV of 1 to 1 is like $25/share") plus a paid stock-promotion account (DDAmanda) repeatedly plugging it. No independently sourced news reachable this cycle. GATED - unconfirmed.
- AIB (AIB Data Centers) $1.74 (+23%, float ~49.6M, only in safety-net scan): Stocktwits references a real-sounding 9/24 Spartanburg SC council meeting on data-center zoning/megawatt allowances, with users themselves asking "what's the news here?" - i.e. even the crowd isn't sure. Could not corroborate via Robinhood or Stocklake this cycle (both unavailable). GATED - unconfirmed, worth re-checking next cycle once news tools recover.
- SHMD (Schmid Group) $4.16 (+15%, float 29.3M): one Stocktwits user claims "Nvidia collaboration, could be in the billions" with a $11+ target; another user in the same thread calls it "scam." Textbook unverified claim - exactly the pattern the hard gate exists to catch. No corroboration available this cycle. GATED - unconfirmed.
Nothing alerted. Re-verify AIB/SHMD/AVX next cycle once get_equity_news is confirmed working again.

## 2026-09-25 16:17 ET — scanner cycle (quiet, no alert; last fire of the day)
Ignition 84 / Warrior 17 / safety-net 20. SPY closed at 771.30 (19:59:59Z regular close). get_equity_news (Robinhood/Benzinga) remained broken all cycle (persistent "tool cannot be found" even after reload) - catalyst checks below are Stocktwits + SEC filing index only.
- ACET (Adicet Bio) $10.09 (+22%, float 9.3M): Stocktwits quotes what reads like a press release - "Adicet Bio to Host Webcast to Present Prula-cel Clinical Data... Monday, September 28, 2026" - a real, specific, dated future event. Checked SEC filing index: NO matching filing found since 9/23. Could not independently corroborate via Robinhood news (outage) or Stocklake (not tried, already capped earlier). IMPORTANT: even if true, this is a forward-looking binary biotech data readout (Monday), not a same-day catalyst for today's move - materially different risk profile (event risk, not momentum). GATED - unconfirmed AND wrong shape of catalyst for this scan's purpose even if it were confirmed.
- TVRD (Tvardi Therapeutics) $1.27 (+20%, float 13.3M): Stocktwits users themselves asking "what's going on here?" with no news cited; most recent substantive item is Q2 earnings from 8/14 (old). No catalyst. GATED.
